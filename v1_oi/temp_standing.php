<?php
	...
	/*else if (!db_contest_running($cid)) {
		...
		$probque="select pid from contest_problem where cid=$cid";
		...
		for ($i=0;$i<$boardnum;$i++) {
			...
			echo "<td class=standing width=". 100/($probnum+4) ."%><a href=userinfo.php?name=".$board[$i]['username'].">".$board[$i]['nickname']."</a></td>";
