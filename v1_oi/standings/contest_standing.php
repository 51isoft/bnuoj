...
//		echo microtime()."<br/>";
		...
		while($rows = @mysql_fetch_array($que)) {
			...
			for($i = 0 ; $i<$n-4 ;$i++){
				...
				if ($lockt=='0000-00-00 00:00:00'||$nowtime>$endt) $sql_ac ="select UNIX_TIMESTAMP(time_submit),runid from status where time_submit<='$endt' and time_submit>='$startt' and contest_belong=$cid and username='$rows[0]' and pid=$pid[$i] and result='$rri' and runid <=' ".$board[$mark][3*$i+5]."'";
				...
				if ($lockt=='0000-00-00 00:00:00'||$nowtime>$endt) $sql_nac ="select count(*) from status where time_submit<='$endt' and time_submit>='$startt' and contest_belong=$cid and username='$rows[0]' and pid=$pid[$i] and result!='$rri' and runid <= ' ".$board[$mark][3*$i+5]."'";
