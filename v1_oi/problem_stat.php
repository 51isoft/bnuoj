...
<center>
<body>
<h3>题目：<a href="problem_show.php?pid=<?php echo $pid; ?>"><?php echo $pid; ?></a>的统计数据</h3>
...
<table width=98%>
	...
	<table width=100% class=pstat>
		<tr>
		<td class=pstat>总共提交数:</td><td class=pstat><?php echo "<a href='status.php?showpid=$pid' class=pstat>".$ntot."</a>"; ?></td>
		</tr>
		<tr>
		<td class=pstat>通过数:</td><td class=pstat><?php echo "<a href='status.php?showpid=$pid&showres=Accepted' class=pstat>".$nac."</a>"; ?></td>
		</tr>
		<tr>
		<td class=pstat>编译错误数:</td><td class=pstat><?php echo "<a href='status.php?showpid=$pid&showres=Compile+Error' class=pstat>".$nce."</a>"; ?></td>
		</tr>
<!--		<tr>
		...
		<tr>
		<td class=pstat>Runtime Error:</td><td class=pstat><?php echo "<a href='status.php?showpid=$pid&showres=Runtimen+Error' class=pstat>".$nre."</a>"; ?></td>
		</tr>
		<tr>
		...
		<tr> -->
		<td class=pstat>未通过数:</td><td class=pstat><?php echo "<a href='status.php?showpid=$pid&showres=Unaccepted' class=pstat>".$not."</a>"; ?></td>
		</tr>
	</table>
	</td>
	<td width=60%>
	<table width=100% class=pstat>
<?php
	$numq="select count(*) from (select * from status where result like 'Accepted%' and pid=$pid group by username) status2";
	list($tot)=mysql_fetch_array(mysql_query($numq));
	$query="select count(*),runid,username,time_used,memory_used,language,length(source),time_submit from (select runid,username,time_used,memory_used,language,source,time_submit from status where result like 'Accepted%' and pid=$pid order by time_used,memory_used,length(source),time_submit) status2 group by username order by time_used,memory_used,length(source) limit $start,$pstatuserperpage";
	echo "<caption>";
...
?>
		<tr>
		<th class=pstat>排名</th><th class=pstat>通过次数</th><th class=pstat>ID</th><th class=pstat>用户名</th><th class=pstat>时间</th><th class=pstat>内存</th><th class=pstat>语言</th><th class=pstat>代码长度</th><th class=pstat>时间</th>
<?php
	$rank=$start;
	$result=mysql_query($query);
