...
function addcontest() {
    ...
    $cres=mysql_query("select problem.title from contest_problem,problem where cid=".$mcid." and contest_problem.pid=problem.pid");
    $str=array();
    while ($crow=mysql_fetch_array($cres)) {
        $str[]=trim(strtolower($crow[0]));
    }
    sort($str);
    mysql_query("update contest set allp='".md5(implode($str,"[-,-]"))."' where cid=".$mcid);
}
...
function deal_zjuhtml($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
...
function deal_jhinv($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=1;$i<$unum-2;$i++) {
...
function deal_neerc($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=2;$i<$unum-4;$i++) {
...
function deal_2011shstatus($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum;
    $extinfo=array();
    for ($i=0;$i<$pnum;$i++) $extinfo[strtolower($_POST['extrainfo'][$i])]=$i;
//    var_dump($extinfo);die();
...
function deal_icpcinfostatus($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum;
    $extinfo=array();
    for ($i=0;$i<$pnum;$i++) $extinfo[strtolower($_POST['extrainfo'][$i])]=$i;
//    var_dump($extinfo);die();
...
function deal_pc2sum($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=1;$i<$unum-1;$i++) {
...
function deal_fdulocal2012($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=1;$i<$unum-1;$i++) {
...
function deal_uestc($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=1;$i<$unum;$i++) {
...
function deal_fzuhtml($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=1;$i<$unum;$i++) {
...
function deal_usuhtml($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    ...
    for ($i=0;$i<$unum;$i++) {
        ...
        for ($j=0;$j<$pnum;$j++) {
            ...
            if (strstr($value,"-")!=null) {
                ...
                //echo $uname." ".$_POST['pid'.$j]." ".date("Y-m-d H:i:s",$sttime+tosecond($act)-10)." * $tnum + ".$act."<br />\n";
                insac($tnum,$sttime,tosecond($act),$_POST['pid'.$j],convert_str($uname),$mcid,$sfreq);
            }
        }
    }
...
function deal_sguhtml($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=1;$i<$unum;$i++) {
...
function deal_amt2011($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=2;$i<$unum;$i++) {
...
//                echo $uname." ".$_POST['pid'.$j]." ".$tnum." * ".date("Y-m-d H:i:s",$edtime-10)."<br />\n";
                ...
                $tnum=0;
                $act=tosecond(strstr($value,'(',true));
                $tnum=intval(substr(strstr($value,'('),1,-1));
//                echo $uname." ".$_POST['pid'.$j]." ".date("Y-m-d H:i:s",$sttime+$act-10)." * $tnum + ".date("Y-m-d H:i:s",$sttime+$act)."<br />\n";
                insac($tnum,$sttime,$act,$_POST['pid'.$j],convert_str($uname),$mcid,$sfreq);
            }
...
function deal_nwerc($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=1;$i<$unum-1;$i++) {
...
//                echo $uname." ".$_POST['pid'.$j]." ".$tnum." * ".date("Y-m-d H:i:s",$edtime-10)."<br />\n";
                ...
                $tnum=0;
                $act=tosecond(trim(strstr($value,'(',true)));
                $tnum=intval(strstr(substr(strstr($value,"("),1),")",true));
//                echo $uname." ".$_POST['pid'.$j]." ".date("Y-m-d H:i:s",$sttime+$act-10)." * $tnum + ".date("Y-m-d H:i:s",$sttime+$act)."<br />\n";
                insac($tnum,$sttime,$act,$_POST['pid'.$j],convert_str($uname),$mcid,$sfreq);
            }
...
function deal_phuket($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
...
function deal_spacific($standtable) {
    global $_POST,$sttime,$edtime,$mcid,$pnum,$sfreq;
    $rows=$standtable->find("tr");
    $unum=sizeof($rows);
    for ($i=0;$i<$unum-1;$i++) {
