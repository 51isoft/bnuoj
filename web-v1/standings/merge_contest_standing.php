...
$cid = $_GET['cid'];
$pagetitle="Merge Standing of Contest ".$cid;
...
if (db_contest_exist($cid))
{
    ...
    $scr = "merge_contest_standing_".$cid.".html";
    $pastsec=$nowtime-$t;
...
//$srefresh=0;
    ...
    if ($pastsec<$srefresh&&!($nowtime>$locktu&&$t<$locktu)&&!($nowtime>$fitimeu&&$t<$fitimeu))//60秒更新一�?
    {
...
<?
        ...
        else
        {
            ...
            if ($nowtime>$fitimeu)
            {
                if ($t>$fitimeu)
                {
                    echo "<script>window.location ='merge_contest_standing_$cid.html';</script>";
                    exit;
                }
...
//	$cid = $_GET['cid'];
            ...
            $csql="select * from contest where allp = '$allp' and type=0";
            //echo $csql;
...
//        else echo "<tr class=odd>";
                ...
                echo "<td width='$ro%'>".$Name_ary[$i][6]."</td>";//Penalty
...
<?php
        ...
        file_put_contents("merge_contest_standing_".$cid.".html",$content);
    }
