...
echo "<center>";
include("cmenu.php");
$query = "select count(runid) from status where contest_belong=$cid and result='Accepted'";
$sql = @mysql_query($query);
$row = @mysql_fetch_array($sql);
$end = $start+$numperrow;
echo "<a href='contest_status.php?start=0&cid=$cid'> <strong>Show All</strong> </a><br>";
if ($start!=0) {
	if ($start<$numperrow) echo "<a href='admin_contest_status.php?start=0&cid=$cid'> <strong>&lt;&lt;Previous</strong> </a>";
	else {
		$prev=$start-$numperrow;
		echo "<a href='admin_contest_status.php?start=$prev&cid=$cid'> <strong>&lt;&lt;Previous</strong> </a>";
	}
}
if ($end < $row[0]) {
	echo "&nbsp;&nbsp;&nbsp;&nbsp;<a href='admin_contest_status.php?start=$end&cid=$cid'><strong>Next&gt;&gt;</strong> </a>";
}
$query="select isroot from user where username='$nowuser'";
$result = @mysql_query($query);
list($isroot)=@mysql_fetch_row($result);
$query="select status.username,status.runid,contest_problem.lable,status.result,status.language,status.time_used,status.memory_used,status.time_submit,contest_problem.cpid from status,contest_problem where status.contest_belong='$cid' and contest_problem.cid=status.contest_belong and contest_problem.pid=status.pid and status.result='Accepted' order by runid desc limit $start,$numperrow";
$result = @mysql_query($query);
echo "<table width=98%>";
echo "<tr>";
...
while (list($uname,$runid,$lable,$res,$lang,$timeused,$memused,$timesubmit,$cpid)=@mysql_fetch_row($result)) {
	...
	$clength=strlen($src);
	echo "<td><center>$clength Byte</center></td>";
