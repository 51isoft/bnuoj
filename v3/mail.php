    ...
    <div id="newmailwindow" class="modal hide fade">
      ...
      <form action="ajax/mail_send.php" method="post" id="mailsend" class="ajform">
        <div class="modal-body">
          ...
          <input type="text" name="title" id="mailtitle" value="" class="input-block-level" placeholder="Mail title"  />
          <textarea rows="12" name="content" id="newmailcontent"  class="input-block-level" placeholder="Input the content here..." onKeyUp="if(this.value.length > <?=$config["limits"]["max_mail_len"]?>) this.value=this.value.substr(0,<?=$config["limits"]["max_mail_len"]?>)"></textarea>
        </div>
...
<script type="text/javascript" src="js/mail.js?<?php echo filemtime("js/mail.js"); ?>"></script>
<?php
include("footer.php");
?>
