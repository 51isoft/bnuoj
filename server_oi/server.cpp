...
void result_dealer(char filename[200],int temp_pid,int temp_runid,int temp_cid,char temp_username[])
{
    ...
    mysql_query(mysql,update);
    if (strstr(resu,"Accepted")!=NULL) {
        sprintf(update,"SELECT count(*) FROM status WHERE username='%s' AND pid='%d' AND result like 'Accepted%%'",temp_username,temp_pid);
        mysql_query(mysql,update);
        res=mysql_use_result(mysql);
        row=mysql_fetch_row(res);
        if (atoi(row[0])==1) {
        ...
        char tempce[MAX_DATA_SIZE]={0};
        char ce_info_data[MAX_DATA_SIZE]={0};
        char ceupdate[MAX_DATA_SIZE]={0};
        writelog("GOT CE\n");
        fgets(tempce,MAX_DATA_SIZE,target_file);
        while (1) {
            fgets(tempce,MAX_DATA_SIZE,target_file);
...
int main(int argc, char * argv[])
{
    ...
    if (argc>1&&strcmp(argv[1],"sj")==0) {
        ...
        sprintf(query,"SELECT runid FROM status,problem WHERE result='Waiting' and isvirtual=0 and status.pid=problem.pid order by runid");
    }
    int temp_t=mysql_real_query(mysql,query,strlen(query));
    if (temp_t) {
