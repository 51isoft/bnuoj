...
if (db_contest_exist($cid)&&db_user_match($nowuser,$nowpass)&&db_user_isroot($nowuser))
{
    ...
    $scr = "contest_standing_admin_only_".$cid.".html";
    $pastsec=$nowtime-$t;
    if ($pastsec<$srefresh&&!($nowtime>$locktu&&$t<$locktu)&&!($nowtime>$fitimeu&&$t<$fitimeu))//60秒更新一次
    {
        Header("Location: contest_standing_admin_only_".$cid.".html");
    }
...
//	$cid = $_GET['cid'];
            ...
            for ($i = 0 ; $i < $id ; $i++)
            {
                ...
                $get = 0;
                //echo $ary[$i][username];
                //$ary[$i][username]=strtolower($ary[$i][username]);
                for ($j = 0 ; $j < $iid ; $j++)
                {
                ...
                if ($k == 1)
                {
                    //echo $ary[$i][username]." $get ".$map[$ary[$i][pid]]." Yes<br/>";
...
//            echo "<table width='98%'>";
		...
		if ($i%2==0) echo "<tr class=even>";
                ...
                "<td width='$ro%' class='nowrap'><a alt='".$Name_ary[$i][username]."' href=../userinfo.php?name=".$Name_ary[$i][username].">".$Name_ary[$i][username]."</a></td>".//nickname
...
<?php
        ...
        file_put_contents("contest_standing_admin_only_".$cid.".html",$content);
    }
