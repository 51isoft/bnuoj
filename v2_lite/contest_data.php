...
include_once("conn.php");
    ...
    $aColumns = array( 'cid', 'title', 'start_time', 'end_time', 'isprivate', 'owner','isvirtual','type','has_cha','challenge_end_time','challenge_start_time' );
    $sIndexColumn = "cid";
    $sTable = "contest";
    ...
    for ( $i=0 ; $i<count($aColumns) ; $i++ )
    {
        if ( $_GET['bSearchable_'.$i] == "true" && $_GET['sSearch_'.$i] != '' )
        {
            ...
            if ($aColumns[$i]=="isvirtual") $sWhere .= $aColumns[$i]." = '".mysql_real_escape_string($_GET['sSearch_'.$i])."' ";
...
//    echo $sQuery;die();
    ...
    while ( $aRow = mysql_fetch_array( $rResult ) )
    {
        //var_dump($aRow);
        $row = array();
        if ($aRow[7]==1) $aRow['title']="<span style='color:blue'>[CF]</span> ".$aRow['title'];
        else if ($aRow[7]==99) $aRow['title']="<span style='color:blue'>[Replay]</span> ".$aRow['title'];
        for ( $i=0 ; $i<count($aColumns) ; $i++ )
        {
            if ($aColumns[$i]=='end_time') {
                if ($aRow['has_cha']==1) $row[] = $aRow[ 9 ];
                else $row[] = $aRow[ $aColumns[$i] ];
                $nowt=date("Y-m-d H:i:s");
                if ($nowt<$aRow['start_time']) $row[]="<span class='cscheduled'>Scheduled</span>";
                else if ($aRow['has_cha']==1&&$nowt>$aRow['end_time']&&$nowt<$aRow[10]) $row[]="<span class='crunning'>Intermission</span>";
                else if ($aRow['has_cha']==1&&$nowt>$aRow[10]&&$nowt<$aRow[9]) $row[]="<span class='crunning'>Challenging</span>";
                else if ($nowt<$aRow['end_time']) $row[]="<span class='crunning'>Running</span>";
                else $row[]="<span class='cpassed'>Passed</span>";
            }
            else if ($aColumns[$i] == "cid" ||$aColumns[$i] == "title" ) {
