          ...
          if($.trim(data)=='Submitted.') //if correct login detail
          {
                $("#submitmsgbox").fadeTo(100,0.1,function()  //start fading the messagebox
                {
                  $(this).html('Success!').addClass('normalmessageboxok').fadeTo(500,1,function() {
                    ...
                    $.get("contest_status.php",{cid: "<?php echo $cid; ?>", randomid: Math.random()},statusfunc);
                  });
                });
...
var showpfunc=function() {
    ...
    $.get("contest_prob.php",{cpid: $(this).attr("name")},function(data) {
        ...
        $(".showstatus").click(function() {
            ...
            $.get("contest_status.php",{cid: "<?php echo $cid; ?>", randomid: Math.random()},function(data) {
                statusfunc(data);
...
var statusfunc=function(data) {
    ...
    oTable = $('#statustable').dataTable( {
        ...
        "bServerSide": true,
        "sDom": '<"H"p>rt<"F"ilp>',
        "sAjaxSource": "contest_status_data.php?cid=<?php echo $cid; ?>&randomid="+Math.random(),
        "sPaginationType": "full_numbers" ,
        "iDisplayLength": <?php echo $statusperpage; ?>,
        "bLengthChange": false,
        "oLanguage": {
...
$.get("contest_info.php",{cid: "<?php echo $cid; ?>", randomid: Math.random()},defaultfunc);
...
$("#cstand_a").click(function() {
    $.get("contest_standing.php",{cid: "<?php echo $cid; ?>", randomid: Math.random()},function(data){
        $("#contest_content").html(data);
...
$("#creport_a").click(function() {
    $.get("contest_report.php",{cid: "<?php echo $cid; ?>", randomid: Math.random()},function(data) {
        $("#contest_content").html(data);
