...
string getResFromFile(char *filename) {
    ...
    while (fgets(tmps,1000000,fp)) {
        res=tmps;
        if (res.find("<div class=\"list\">")!=string::npos) {
            while (fgets(tmps,1000000,fp)) {
                res+=tmps;
                if (strstr(tmps,"</ul>")) break;
            }
            break;
        }
    }
...
bool login() {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_COOKIEJAR, "uestc.cookie");
        curl_easy_setopt(curl, CURLOPT_URL, "http://acm.uestc.edu.cn/login_action.php");
        string post=(string)"username="+username+"&password="+password+"&x=0&y=0";
    ...
    //cout<<ts;
    if (ts.find("Author's ID or password is incorrect.")!=string::npos) return false;
    return true;
}
...
bool submit(string pid,string lang,string source) {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1);
        curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "uestc.cookie");
        curl_easy_setopt(curl, CURLOPT_URL, ((string)"http://acm.uestc.edu.cn/submit_action.php?pid="+pid+"&cid=0").c_str());
/*
language=0&
code=%23include+%3Cstdio.h%3E++%0D%0Aint+main%28%29+++%0D%0A%7B++%0D%0Aint+a%2C+b%3B++%0D%0Ascanf%28%22%25d+%25d%22%2C%26a%2C+%26b%29%3B++%0D%0Aprintf%28%22%25d%22%2C+a%2Bb%29%3B++%0D%0Areturn+0%3B++%0D%0A%7D++
*/

        string post=(string)"language="+lang+"&code="+escapeURL(source);
        //cout<<post;
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
        res = curl_easy_perform(curl);
        curl_easy_cleanup(curl);
    ...
    string ts=getAllFromFile(tfilename);
    //cout<<ts;
    if (ts.find("The problem id is not existed.")!=string::npos||ts.find("The page is temporarily unavailable")!=string::npos) return false;
    return true;
}
...
string getRunid(string s) {
    ...
    while (s[pos]!='>') pos++;
    pos++;
    while (s[pos]==' '||s[pos]=='\r'||s[pos]=='\t'||s[pos]=='\n') pos++;
    int st=pos;
    while (s[pos]!=' '&&s[pos]!='\r'&&s[pos]!='\t'&&s[pos]!='\n') pos++;
    return s.substr(st,pos-st);
}
...
string getCEinfo(string runid) {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_COOKIEJAR, "uestc.cookie");
        string url=(string)"http://acm.uestc.edu.cn/submission.php?sid="+runid;
        //cout<<url;
    ...
    int pos=ts.find("</head>");
    while (ts[pos]!='>') pos++;
    pos++;
    while (ts[pos]==' '||ts[pos]=='\r'||ts[pos]=='\t'||ts[pos]=='\n') pos++;
    int st=pos;
    pos=ts.find("</html>");
...
//http://acm.uestc.edu.cn/status.php?id=bnuvjudge&pid=1000&language=0
bool getStatus(string pid,string lang,string & result,string& ce_info,string &tu,string &mu) {
    ...
    while (true) {
        ...
        if(curl) {
            ...
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(curl, CURLOPT_COOKIEJAR, "uestc.cookie");
            string url=(string)"http://acm.uestc.edu.cn/status.php?id="+username+"&pid="+pid+"&language="+lang;
            //cout<<url;
            curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
            res = curl_easy_perform(curl);
            curl_easy_cleanup(curl);
