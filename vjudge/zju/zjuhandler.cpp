...
string getResFromFile(char *filename)
{
    ...
    while (fgets(tmps,1000000,fp))
    {
        ts=tmps;
        cnt++;
        if (ts.find("<tr class=\"rowOdd\">")!=string::npos)
        {
            res=tmps;
            while (1) {
...
bool login() {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_COOKIEJAR, "zju.cookie");
    ...
    //cout<<ts;
    if (ts.find("Handle or password is invalid.")!=string::npos) return false;
    return true;
}
...
string getRealPid(string pid) {
    ...
    while (fgets(tmps,1000000,fp)) {
        s=tmps;
        if (s.find("submit.do?problemId=")!=string::npos) break;
    }
    fclose(fp);
    int pos=s.find("submit.do?problemId=");
    while (s[pos]!='=') pos++;
    pos++;
    int st=pos;
    while (s[pos]!='\"') pos++;
...
bool submit(string pid,string lang,string source) {
    ...
    if(curl) {
        curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "zju.cookie");
        curl_easy_setopt(curl, CURLOPT_URL, "http://acm.zju.edu.cn/onlinejudge/submit.do");
/*
problemId=1&languageId=2&source=kijtygjhmbu64765tugfhre43556789hgfjkl
*/

        string post=(string)"problemCode="+pid+"&languageId="+lang+"&source="+escapeURL(source);
        //cout<<post;
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
        res = curl_easy_perform(curl);
        curl_easy_cleanup(curl);
    ...
    string ts=getAllFromFile(tfilename);
    //cout<<ts;
    if (ts.find("Submit Successfully</div>")==string::npos) {
        cout<<ts;
        return false;
    }
    return true;
...
string getResult(string s) {
    int pos=s.find("<td class=\"runJudgeStatus\">");
    while (s[pos]!='>') pos++;
    pos++;
    while (s[pos]!='>') pos++;
    pos++;
...
string getRunid(string s) {
    int pos=s.find("The submission id is");
    while (s[pos]!='>') pos++;
    pos++;
    int st=pos;
    while (s[pos]!='<') pos++;
...
string getCEinfo(string runid) {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "zju.cookie");
        string url=(string)"http://acm.zju.edu.cn/onlinejudge/showJudgeComment.do?submissionId="+runid;
        //cout<<url;
...
string getUsedMem(string s) {
    int pos=s.find("<td class=\"runMemory\">");
    while (s[pos]!='>') pos++;
    pos++;
    int st=pos;
    while (s[pos]!='<') pos++;
...
string convertResult(string res) {
    ...
    if (res.find("Segmentation Fault")!=string::npos) return "Runtime Error";
	...
	if (res.find("Non-zero Exit Code")!=string::npos) return "Runtime Error";
	if (res.find("Floating Point Error")!=string::npos) return "Runtime Error";
    if (res.find("Output Limit Exceeded")!=string::npos) return "Output Limit Exceed";
    return res;
}

bool getStatus(string pid,string lang,string & result,string& ce_info,string &tu,string &mu) {
    int begin=time(NULL);
    string runid=getRunid(getAllFromFile(tfilename));
    tu=mu="0";
    string ts;
    while (true) {
        FILE * fp=fopen(tfilename,"w+");
        ...
        if(curl) {
            ...
            curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "zju.cookie");
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
            string url=(string)"http://acm.zju.edu.cn/onlinejudge/showRuns.do?contestId=1&idStart="+runid+"&idEnd="+runid;
            //cout<<url;
            curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
            res = curl_easy_perform(curl);
            curl_easy_cleanup(curl);
