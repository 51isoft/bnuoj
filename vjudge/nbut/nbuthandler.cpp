...
string getPara() {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_URL,"http://acm.nbut.cn/User/login.xhtml");
...
bool getStatus(string pid,string lang,string & result,string& ce_info,string &tu,string &mu) {
    ...
    while (true) {
        ...
        if(curl) {
            ...
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "nbut.cookie");
            string url=(string)"http://acm.nbut.cn/Problem/status.xhtml?username="+username+"&problemid="+pid+"&language="+lang;
            //cout<<url<<endl;
            curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
            res = curl_easy_perform(curl);
            curl_easy_cleanup(curl);
