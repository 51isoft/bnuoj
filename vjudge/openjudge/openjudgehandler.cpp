...
bool login() {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_COOKIEJAR, "openjudge.cookie");
        curl_easy_setopt(curl, CURLOPT_URL, "http://poj.openjudge.cn/api/auth/login/");
        string post=(string)"email="+username+"&password="+password;
    ...
    cout<<ts;
    if (ts.find("\"result\":\"ERROR\"")!=string::npos) return false;
    return true;
}
...
bool submit(string pid,string lang,string source) {
    ...
    if(curl) {
        curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "openjudge.cookie");
        curl_easy_setopt(curl, CURLOPT_URL, "http://poj.openjudge.cn/api/solution/submit/");
/*
contestId=2&problemNumber=1000&language=G%2B%2B&source=dslkjfcsapo'ljfkmas'pl%3Bfas%0Agdsgh%0Adfh%0Adfg%0Ahdfs%0Ah%0Adf%0Ahdf%0Ah%0Ads
*/

        string post=(string)"contestId=2&problemNumber="+pid+"&language="+escapeURL(lang)+"&source="+escapeURL(source);
        //cout<<post;
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
        res = curl_easy_perform(curl);
        curl_easy_cleanup(curl);
    ...
    string ts=getAllFromFile(tfilename);
    cout<<ts;
    if (ts.find("\"result\":\"ERROR\"")!=string::npos||ts.find("The page is temporarily unavailable")!=string::npos) return false;
    return true;
}


string getResult(string s) {
    int pos=s.find("<p class=\"compile-status\">");
    pos+=strlen("<p class=\"compile-status\">");
    while (s[pos]!='>') pos++;
    pos++;
    int st=pos;
    while (s[pos]!='<') pos++;
...
string getRunid(string s) {
    int pos=s.find("/practice\\/solution\\/");
    pos+=strlen("/practice\\/solution\\/");
    int st=pos;
    while (s[pos]!='\\') pos++;
    return s.substr(st,pos-st);
}
...
string getCEinfo(string runid) {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "openjudge.cookie");
        string url=(string)"http://poj.openjudge.cn/practice/solution/"+runid;
        //cout<<url;
...
//http://poj.openjudge.cn/practice/solution/1985794/
...
int main() {
    init();
    judge("1000","1","1",getAllFromFile("test.cpp"));
    return 0;
}
*/
