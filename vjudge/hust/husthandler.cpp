...
string getResFromFile(char *filename) {
    ...
    {
        ts=tmps;
        if (ts.find("<tr align=center class='evenrow'>")!=string::npos)
        {
            res=ts.substr(0,ts.find("</tr>"));
            break;
        }
    }
    fclose(fp);
...
bool login() {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_COOKIEJAR, "hust.cookie");
        curl_easy_setopt(curl, CURLOPT_URL, "http://acm.hust.edu.cn/login.php");
        string post=(string)"submit=Submit&user_id="+username+"&password="+password;
    ...
    //cout<<ts;
    if (ts.find("alert(")!=string::npos) return false;
    return true;
}
...
bool submit(string pid,string lang,string source) {
    ...
    if(curl) {
        curl_easy_setopt(curl, CURLOPT_WRITEDATA, fp);
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "hust.cookie");
        curl_easy_setopt(curl, CURLOPT_URL, "http://acm.hust.edu.cn/submit.php");
/*
id=1000&language=1&source=fsafasfasgdsgsdgsdg%0D%0Ae%0D%0Agdf%0D%0Ag%0D%0Adgd%0D%0Asfgd
*/

        string post=(string)"id="+pid+"&submit=Submit&language="+lang+"&source="+escapeURL(source);
        //cout<<post;
        curl_easy_setopt(curl, CURLOPT_POSTFIELDS, post.c_str());
        res = curl_easy_perform(curl);
        curl_easy_cleanup(curl);
...
string getCEinfo(string runid) {
    ...
    if(curl) {
        ...
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
        curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "hust.cookie");
        string url=(string)"http://acm.hust.edu.cn/ceinfo.php?sid="+runid;
        //cout<<url;
...
//http://acm.hust.edu.cn/status.php?problem_id=1000&user_id=bnuvjudge&language=1&jresult=-1
bool getStatus(string pid,string lang,string & result,string& ce_info,string &tu,string &mu) {
    ...
    while (true) {
        ...
        if(curl) {
            ...
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, NULL);
            curl_easy_setopt(curl, CURLOPT_COOKIEFILE, "hust.cookie");
            string url=(string)"http://acm.hust.edu.cn/status.php?problem_id="+pid+"&user_id="+username+"&language="+lang;
            //cout<<url;
            curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
            res = curl_easy_perform(curl);
            curl_easy_cleanup(curl);
        ...
        if (result.find("Waiting")==string::npos
            ...
            &&result.find("Pending")==string::npos
...
void judge(string pid,string lang,string runid,string src) {
    ...
    if (!submit(pid,lang,src)) {
        ...
        if (!submit(pid,lang,src)) {
            writelog("Assume should wait a while. Sleep 5 seconds.\n");
            usleep(5000000);
            if (!submit(pid,lang,src)) {
